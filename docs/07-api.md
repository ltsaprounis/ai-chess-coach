...
| GET    | `/api/players/{u}/profile`             | The student profile (docs/06-coach.md, "Player profile"): facts always fresh, narrative and its metadata (`agent_id`, `prompt_version`, `generated_at`, `games_covered` at generation) from storage — never an LLM call. The response also carries the **current** `prompt_version`, against the narrative's own, so a template bump actually flags the stored text stale (it could not before: nothing sent the current one). Facts take two passes — `list_game_summaries` over the caller's whole scope to pick the level window and build the full-archive trajectory, then `list_analyzed_games` narrowed to it (docs/06-coach.md, "Window"); the bound only ever tightens the caller's own window, never widens it. Query `since`/`until`/`time_class` as `/report`, with one asymmetry: **facts honour all three, the narrative is looked up by `time_class` alone** (it is stored under no window — see 06-coach.md, "Why time control keys it and the window does not"). `narrative_games_now` reports the live analyzed count for the *narrative's* scope, so the UI compares like with like instead of flagging every windowed view as stale. Unknown player: empty facts and no narrative, 200 like `/report` |
| POST   | `/api/players/{u}/profile`             | Regenerate the narrative (user-triggered — LLM calls cost money; GET never generates): body `{agent_id?, time_class?}` (default agent otherwise, 400 on unknown id); fresh facts for that control → `render_profile_prompt(has_tools=True)` → `complete(toolkit=…)` → `save_player_profile` under `(username, time_class)`; returns the same shape as GET. The run is **agentic** (docs/06-coach.md, "Narrative"): it gets chat's read-only toolkit, scoped to the same window as the facts so every count in the document shares one denominator, seeded with the facts' own comparisons so a question it asks joins their BH family. Needs the engine pool for `analyze_position`; without one the toolkit simply carries no analyst. 409 with no analyzed games in that scope to describe, naming the control |
...
- Player profile (docs/06-coach.md, "Player profile"): GET assembles
  fresh facts in the threadpool like `/report`, in two passes —
  `list_game_summaries` over the caller's scope picks the level window
  (`profile_window`) and builds the full-archive `build_trajectory`,
  then `list_analyzed_games` narrowed to that bound feeds
  `build_report` → `build_profile` — and attaches the stored narrative
  row when one exists; POST regenerates agentically through the chosen
  provider and saves. Both lists go
